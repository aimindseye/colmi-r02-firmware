#!/usr/bin/env python3
"""
Trace the RY02 six-byte configuration/identifier pipeline.

Focus:
  * source-1/D0 six-byte state updater 0x00828DD2;
  * startup/configuration producer 0x00829E70;
  * common apply/submit helper 0x008385F8;
  * record-field extractor 0x00839CA4 and parser 0x00838AFC;
  * supporting helpers 0x00838C94, 0x00829C94, and 0x00829FF2;
  * generic size distributions for low targets 0x0003F7A8 and 0x0003F848;
  * exact references in RAM window 0x00208640..0x002086E0;
  * heuristic .33 counterparts.

The report is descriptive. It does not identify the six-byte value as a BLE
address, MAC address, bond record, or device identity without corroborating
protocol evidence.
"""
...
COMPARE_TARGET = 0x0003F7A8
COPY_TARGET = 0x0003F848

RAM_WINDOW_START = 0x00208640
RAM_WINDOW_END = 0x002086E0
CALLER_SAVED = {ARM_REG_R0, ARM_REG_R1, ARM_REG_R2, ARM_REG_R3}

FUNCTION_TARGETS: tuple[tuple[str, int], ...] = (
    ("source1_D0_six_byte_state_update", 0x00828DD2),
    ("source1_D0_startup_configuration", 0x00829E70),
    ("six_byte_apply_submit_candidate", 0x008385F8),
    ("record_six_byte_field_extractor_candidate", 0x00839CA4),
    ("record_locator_parser_candidate", 0x00838AFC),
    ("configuration_structure_builder_candidate", 0x00838C94),
    ("configuration_followup_candidate", 0x00829C94),
    ("six_byte_state_sync_candidate", 0x00829FF2),
)

KNOWN_FLOW_SITES: tuple[tuple[str, int], ...] = (
    ("state_compare", 0x00828DE0),
    ("state_copy", 0x00828DEE),
    ("state_apply", 0x00828E00),
    ("state_publish_D0", 0x00828E08),
    ("config_extract", 0x00829EB2),
    ("config_copy_source", 0x00829EC8),
    ("config_compare", 0x00829EE8),
    ("config_apply", 0x00829EF2),
    ("config_publish_D0", 0x00829EFA),
    ("apply_helper_copy_input", 0x00838604),
    ("extractor_copy_field", 0x00839CE8),
)
...
def normalized_token(insn, image: Image) -> str:
    ...
    if resolved is not None:
        _, value = resolved
        if RAM_WINDOW_START <= value < RAM_WINDOW_END:
            parts.append("LIT_RAM_WINDOW")
...
def report_function(
    ...
    for insn in graph.ordered():
        ...
        print(
            f"  insn=0x{insn.address:08X} "
            f"literal=0x{literal:08X} value=0x{value:08X}"
        )

    print("reachable CFG:")
    for insn in graph.ordered():
        marker = ">>" if insn.address in {site for _, site in KNOWN_FLOW_SITES} else "  "
        print(format_instruction(image, insn, marker))
...
def immediate_r2_before_call(image: Image, call_address: int) -> int | None:
    sequence = contiguous_predecessors(image, call_address, window=0x50)
    for insn in reversed(sequence):
        if is_call(insn):
            return None
        try:
            ...
            written = []
        if ARM_REG_R2 not in written:
            continue
        if (
            insn.mnemonic in {"mov", "movs"}
            and len(insn.operands) >= 2
            and insn.operands[0].type == ARM_OP_REG
            and insn.operands[0].reg == ARM_REG_R2
...
def report_generic_size_distribution(image: Image, label: str, target: int) -> None:
    callers = scan_direct_callers(image, target)
    distribution: collections.Counter[int] = collections.Counter()
    unknown = 0

    for caller in callers:
        size = immediate_r2_before_call(image, caller)
        if size is None:
            unknown += 1
        else:
            distribution[size] += 1
    ...
    print("=" * 112)
    print(f"generic helper candidate: {label}")
    print(f"target: 0x{target:08X}")
    print(f"direct callers: {len(callers)}")
    print(f"calls with locally proven immediate r2 size: {sum(distribution.values())}")
    print(f"calls with unknown/non-immediate size: {unknown}")
    print("immediate-size distribution:")
    for size, count in sorted(distribution.items()):
        print(f"  size=0x{size:X} ({size}): {count}")
    print("six-byte call sites:")
    for caller in callers:
        if immediate_r2_before_call(image, caller) == 6:
            parent = probable_enclosing_start(image, caller)
            print(
                f"  call=0x{caller:08X} "
                f"parent={f'0x{parent:08X}' if parent is not None else 'unresolved'}"
...
def report_ram_window(image: Image) -> None:
    print("=" * 112)
    print(
        f"RAM WINDOW LITERAL INVENTORY "
        f"0x{RAM_WINDOW_START:08X}..0x{RAM_WINDOW_END - 1:08X}"
    )
    hits = []
    for offset in range(image.code_start, len(image.payload) - 4, 2):
        insn = image.decode_one(offset)
        ...
        literal, value = resolved
        if RAM_WINDOW_START <= value < RAM_WINDOW_END:
            hits.append((insn.address, literal, value))

    by_value: dict[int, list[tuple[int, int]]] = collections.defaultdict(list)
    for site, literal, value in hits:
        by_value[value].append((site, literal))

    print(f"distinct literal values: {len(by_value)}")
    print(f"literal-load sites: {len(hits)}")
    for value in sorted(by_value):
        print(f"value=0x{value:08X}: loads={len(by_value[value])}")
        for site, literal in by_value[value]:
...
def report_known_flow(image: Image) -> None:
    print("=" * 112)
    print("KNOWN SIX-BYTE FLOW SITES")
    for label, address in KNOWN_FLOW_SITES:
        insn = image.decode_one(image.offset_for_runtime(address))
        print()
        ...
        if is_call(insn):
            for register, name in (
                ...
                print(f"  {name}: {local_register_provenance(image, address, register)}")
        print_context(image, address, before=0x20, after=0x34)
    print()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Trace RY02 six-byte configuration/identifier helpers, "
            "RAM window, dataflow sites, and generic compare/copy sizes."
        )
    )
    parser.add_argument(
        "firmware38",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("RY02 SIX-BYTE IDENTIFIER SEMANTICS REPORT")
    ...
    print("  - six-byte width alone does not identify BLE/MAC semantics")
    print("  - 0x3F7A8/0x3F848 labels remain candidates until ABI evidence closes them")
    print("  - only CFG-reachable instructions are assigned to functions")
    print("  - r0-r3 provenance stops at intervening calls")
    print("  - .33 counterpart scores are heuristic")
    print()

    print("# KNOWN FLOW")
    report_known_flow(image38)

    print("# TARGET FUNCTION PROVENANCE")
    for label, address in FUNCTION_TARGETS:
        report_function(image38, label, address, image33)

    print("# GENERIC HELPER SIZE DISTRIBUTIONS")
    report_generic_size_distribution(
        image38,
        "compare_candidate",
        COMPARE_TARGET,
    )
    ...
    for label, target in (
        ("compare_candidate", COMPARE_TARGET),
        ("copy_candidate", COPY_TARGET),
    ):
        callers = scan_direct_callers(image38, target)
        six = sum(
            1
            for caller in callers
            if immediate_r2_before_call(image38, caller) == 6
        )
        print(
            f"helper 0x{target:08X} {label}: "
            f"callers={len(callers)} immediate_size6={six}"
        )
